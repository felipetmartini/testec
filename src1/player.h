...
class Player : public Creature, public Cylinder
{
	public:
		...
		void sendAddTileItem(const Tile* tile, const Position& pos, const Item* item) {
			if (clients.size() > 0) {
				for (auto& client : clients) {
					client->sendAddTileItem(pos, tile->getStackposOfThing(this, item), item);
				}
			}
		}
		void sendUpdateTileItem(const Tile* tile, const Position& pos, const Item* item) {
			if (clients.size() > 0) {
				for (auto& client : clients) {
					client->sendUpdateTileItem(pos, tile->getStackposOfThing(this, item), item);
				}
			}
		}
		...
		void sendCreatureAppear(const Creature* creature, const Position& pos, bool isLogin) {
			if (clients.size() > 0) {
				for (auto& client : clients) {
					client->sendAddCreature(creature, pos, creature->getTile()->getStackposOfThing(this, creature), isLogin);
				}
			}
		}
		...
		void sendCreatureTurn(const Creature* creature) {
			if (clients.size() > 0) {
				for (auto& client : clients) {
					client->sendCreatureTurn(creature, creature->getTile()->getStackposOfThing(this, creature));
				}
			}
		}
			...
			if (clients.size() > 0) {
				if (creature->getPlayer()) {
					...
					if (canSeeInvisibility()) {
						...
						if (visible) {
							for (auto& client : clients) {
								client->sendAddCreature(creature, creature->getPosition(), creature->getTile()->getStackposOfThing(this, creature), false);
							}
						} else {
							for (auto& client : clients) {
								client->sendRemoveTileThing(creature->getPosition(), creature->getTile()->getStackposOfThing(this, creature));
							}
						}
					}
				}
